...
def get_llm() -> LLM:
    ...
    if backend == "openai_compatible":
        ...
        # --- NEW CODE: Auto-fetch fresh Google Token ---
        if "googleapis.com" in base_url and api_key in ("EMPTY", "YOUR_GOOGLE_AUTH_TOKEN"):
            import google.auth
            import google.auth.transport.requests
            
            # Fetch and refresh the token automatically
        ...
        if not raw_model_name.startswith("openai/"):
            # Note: CrewAI/LiteLLM expects the 'openai/' prefix for compatible endpoints
